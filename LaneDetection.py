...
class LaneDetector():
    ...
    def trapezeMask(self, processed_frame):
        # create trapeze mask
        vertices = np.array([[(125, processed_frame.shape[0] - 100),
                              (550, 430), (750, 430),
                              (processed_frame.shape[1] - 125,
                               processed_frame.shape[0] - 100)]],
                            dtype=np.int32)
    ...
    def laneSwitchDetection(self, frame):
        ...
        if self.message_frame_counter <= 90:
    ...
    def preprocess(self, frame):
        ...
        trapeze_frame = self.trapezeMask(frame_HSV)
    ...
    def detect(self, processed_frame, org_frame):  # return left lane mask and right lane mask
        ...
        lines = cv.HoughLinesP(processed_frame, rho=3, theta=np.pi / 160, threshold=20,minLineLength=130, maxLineGap=180)
...
stream = cv.VideoCapture('cropped_sample2.mp4')
